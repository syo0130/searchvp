 ...
 * @param step The step size
 */
fun count(start: Float = 0f, step: Float = 1f): Sequence<Float> =
 ...
 * @param step The step size
 */
fun count(start: Double = 0.0, step: Double = 1.0): Sequence<Double> =
    generateSequence(start) { it + step }
...
fun String.permutations(): Sequence<String> =
    toList().permutations().map { it.joinToString(separator = "") }
        ...
        sequence {
            ...
            while (true) {
                ...
                if (j < n) {
                    ...
                    edge =
                        (edge - 1 downTo 0).firstOrNull { elements[it] < elements[it + 1] } ?: break
